...
It delegates to `text-to-gibberlink.py --ui` so you always get the latest UI.
"""
...
def main() -> int:
    ...
    wrapper = os.path.join(here, "text-to-gibberlink.py")
    if not os.path.exists(wrapper):
        print("Could not find text-to-gibberlink.py next to launcher.py", file=sys.stderr)
        return 2
    try:
